    ...
    @run_list.each do |item|
      if item.start_with?("recipe")
        ...
        unless cookbooks.member?(recipe)
          STDERR.puts "ERROR: '#{item}' is an invalid run_list recipe not managed by spiceweasel"
          exit(-1)
        end
        ...
        unless environments.member?(environment)
          STDERR.puts "ERROR: '#{item}' is an invalid run_list environment not managed by spiceweasel"
          exit(-1)
        end
        ...
        unless roles.member?(role)
          STDERR.puts "ERROR: '#{item}' is an invalid run_list role not managed by spiceweasel"
          exit(-1)
        end
      else
        STDERR.puts "ERROR: '#{item}' is an invalid run_list component"
        exit(-1)
      end
    end
  end
